...
export function CurrencyField({
  ...
  const step = currencyField.step ?? 100;
        ...
        <div style={{ display: 'grid', gap: '0.75rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            {variant === 'stepper-with-slider' ? (
              <button
                type="button"
                onClick={() => onChange(Math.max(currencyField.min, numericValue - step))}
            ...
            {variant === 'stepper-with-slider' ? (
              <button
                type="button"
                onClick={() => onChange(Math.min(currencyField.max, numericValue + step))}
                style={stepButtonStyle}
              >
                +
              </button>
